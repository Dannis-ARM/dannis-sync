...
def run_powershell_script(script_path: Path, capture_output: bool = False) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"✅ PowerShell script completed: {script_path.name}")
...
def run_batch_script(script_path: Path, capture_output: bool = False) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"✅ Batch script completed: {script_path.name}")
        return result.stdout if capture_output else None
    except subprocess.CalledProcessError as e:
...
def run_script(script_path: Path, capture_output: bool = False) -> Optional[str]:
